...
$(QC_DIR)/Sample3.qplot.done: $(FINAL_BAM_DIR)/Sample3.recal.bam.done
	...
	@$(QPLOT_EXE) --reference $(REF) --dbsnp $(DBSNP_VCF) --gccontent $(REF).GCcontent --stats $(basename $@).stats --Rcode $(basename $@).R --minMapQuality 0 --bamlabel Sample3_recal,Sample3_dedup $(basename $^) $(DEDUP_TMP)/Sample3.dedup.bam 2> $(basename $@).log || (echo "`grep -i -e abort -e error -e failed $(basename $@).log`" >&2; echo "Failed QPLOT step" >&2; mkdir -p $(OUT_DIR)/failLogs; cp $(basename $@).log $(OUT_DIR)/failLogs/$(notdir $(basename $@).log); echo "See $(OUT_DIR)/failLogs/$(notdir $(basename $@).log) for more details" >&2; exit 1;)
	rm -f $(basename $@).log
	touch $@
