...
$(OUT_DIR)/Sample3.OK: <outdir_path>/QCFiles/Sample3.genoCheck.done <outdir_path>/QCFiles/Sample3.qplot.done <outdir_path>/bams/Sample3.recal.bam.bai.done <outdir_path>/bams/Sample3.recal.bam.done
	@echo `date +'%F.%H:%M:%S'` touch $@; touch $@

<outdir_path>/QCFiles/Sample3.genoCheck.done: <outdir_path>/bams/Sample3.recal.bam.done <outdir_path>/bams/Sample3.recal.bam.bai.done
	mkdir -p $(@D)
...
<outdir_path>/QCFiles/Sample3.qplot.done: <outdir_path>/bams/Sample3.recal.bam.done
	mkdir -p $(@D)
...
<outdir_path>/bams/Sample3.recal.bam.bai.done: <outdir_path>/bams/Sample3.recal.bam.done
	mkdir -p $(@D)
...
<outdir_path>/bams/Sample3.recal.bam.done: <outdir_path>/tmp/alignment.pol/Sample3.merged.bam.done
	mkdir -p $(@D)
...
<outdir_path>/tmp/alignment.pol/fastq/Sample_3/File1_R1.bam.done: <outdir_path>/tmp/alignment.aln/fastq/Sample_3/File1_R1.bam.done
	mkdir -p $(@D)
...
<outdir_path>/tmp/alignment.pol/Sample3.merged.bam.done: <outdir_path>/tmp/alignment.pol/fastq/Sample_3/File1_R1.bam.done 
	mkdir -p $(@D)
	@echo `date +'%F.%H:%M:%S'`" ln $(basename $^) $(basename $@)"
...
<outdir_path>/tmp/alignment.aln/fastq/Sample_3/File1_R1.bam.done: <outdir_path>/tmp/bwa.sai.t/fastq/Sample_3/File1_R1.sai.done <outdir_path>/tmp/bwa.sai.t/fastq/Sample_3/File1_R2.sai.done
	mkdir -p $(@D)
	@echo `date +'%F.%H:%M:%S'`" (<gotcloud_root>/bin/bwa sampe -r  "@RG\tID:RGID3\tSM:SampleID3\tLB:Lib3\tCN:UM\tPL:ILLUMINA" <gotcloud_root>/test/chr20Ref/human_g1k_v37_chr20.fa $(basename $^) <gotcloud_root>/test/align/fastq/Sample_3/File1_R1.fastq.gz <gotcloud_root>/test/align/fastq/Sample_3/File1_R2.fastq.gz | <gotcloud_root>/bin/samtools view -uhS - | <gotcloud_root>/bin/samtools sort -m 2000000000 - -o $(basename $@) -T $(basename $(basename $@))) 2> $(basename $(basename $@)).sampe.log"
	@(<gotcloud_root>/bin/bwa sampe -r  "@RG\tID:RGID3\tSM:SampleID3\tLB:Lib3\tCN:UM\tPL:ILLUMINA" <gotcloud_root>/test/chr20Ref/human_g1k_v37_chr20.fa $(basename $^) <gotcloud_root>/test/align/fastq/Sample_3/File1_R1.fastq.gz <gotcloud_root>/test/align/fastq/Sample_3/File1_R2.fastq.gz | <gotcloud_root>/bin/samtools view -uhS - | <gotcloud_root>/bin/samtools sort -m 2000000000 - -o $(basename $@) -T $(basename $(basename $@))) 2> $(basename $(basename $@)).sampe.log || (echo "`grep -i -e abort -e error -e failed $(basename $(basename $@)).sampe.log`" >&2; echo "Failed sampe step" >&2; mkdir -p $(OUT_DIR)/failLogs; cp $(basename $(basename $@)).sampe.log $(OUT_DIR)/failLogs/$(notdir $(basename $(basename $@)).sampe.log); echo "See $(OUT_DIR)/failLogs/$(notdir $(basename $(basename $@)).sampe.log) for more details" >&2; exit 1;)
	rm -f $(basename $(basename $@)).sampe.log
	@echo `date +'%F.%H:%M:%S'` touch $@; touch $@
	rm -f $(basename $^)

<outdir_path>/tmp/bwa.sai.t/fastq/Sample_3/File1_R1.sai.done:
	mkdir -p $(@D)
	@echo `date +'%F.%H:%M:%S'`" <gotcloud_root>/bin/bwa aln -q 15 -t 1 <gotcloud_root>/test/chr20Ref/human_g1k_v37_chr20.fa <gotcloud_root>/test/align/fastq/Sample_3/File1_R1.fastq.gz -f $(basename $@) 2> $(basename $@).log"
...
<outdir_path>/tmp/bwa.sai.t/fastq/Sample_3/File1_R2.sai.done:
	mkdir -p $(@D)
	@echo `date +'%F.%H:%M:%S'`" <gotcloud_root>/bin/bwa aln -q 15 -t 1 <gotcloud_root>/test/chr20Ref/human_g1k_v37_chr20.fa <gotcloud_root>/test/align/fastq/Sample_3/File1_R2.fastq.gz -f $(basename $@) 2> $(basename $@).log"
	@<gotcloud_root>/bin/bwa aln -q 15 -t 1 <gotcloud_root>/test/chr20Ref/human_g1k_v37_chr20.fa <gotcloud_root>/test/align/fastq/Sample_3/File1_R2.fastq.gz -f $(basename $@) 2> $(basename $@).log || (echo "`grep -i -e abort -e error -e failed $(basename $@).log`" >&2; echo "Failed aln step" >&2; mkdir -p $(OUT_DIR)/failLogs; cp $(basename $@).log $(OUT_DIR)/failLogs/$(notdir $(basename $@).log); echo "See $(OUT_DIR)/failLogs/$(notdir $(basename $@).log) for more details" >&2; exit 1;)
	rm -f $(basename $@).log
	@echo `date +'%F.%H:%M:%S'` touch $@; touch $@
