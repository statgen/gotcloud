...
$(OUT_DIR)/Sample2.OK: $(QC_DIR)/Sample2.genoCheck.done $(QC_DIR)/Sample2.qplot.done $(FINAL_BAM_DIR)/Sample2.recal.bam.bai.done $(FINAL_BAM_DIR)/Sample2.recal.bam.done
...
$(QC_DIR)/Sample2.genoCheck.done: $(FINAL_BAM_DIR)/Sample2.recal.bam.done $(FINAL_BAM_DIR)/Sample2.recal.bam.bai.done
	mkdir -p $(@D)
	@echo `date +'%F.%H:%M:%S'`" $(VERIFY_BAM_ID_EXE) --bam $(basename $<) --out $(basename $@) --verbose --vcf $(HM3_VCF)   2> $(basename $@).log"
...
$(FINAL_BAM_DIR)/Sample2.recal.bam.done: $(DEDUP_TMP)/Sample2.merged.bam.done
	mkdir -p $(@D)
	@echo `date +'%F.%H:%M:%S'`" $(BAM_EXE) dedup --log $(basename $@).metrics  --recab --in $(basename $^) --out $(basename $@) --refFile $(REF) --dbsnp $(DBSNP_VCF) --storeQualTag OQ  --phoneHomeThinning 0 2> $(basename $@).log"
	@$(BAM_EXE) dedup --log $(basename $@).metrics  --recab --in $(basename $^) --out $(basename $@) --refFile $(REF) --dbsnp $(DBSNP_VCF) --storeQualTag OQ  --phoneHomeThinning 0 2> $(basename $@).log || (echo "`grep -i -e abort -e error -e failed $(basename $@).log`" >&2; echo "Failed recab step" >&2; mkdir -p $(OUT_DIR)/failLogs; cp $(basename $@).log $(OUT_DIR)/failLogs/$(notdir $(basename $@).log); echo "See $(OUT_DIR)/failLogs/$(notdir $(basename $@).log) for more details" >&2; exit 1;)
	rm -f $(basename $@).log
	@echo `date +'%F.%H:%M:%S'` touch $@; touch $@
	rm -f $(basename $^)
...
$(ALN_TMP)/fastq/Sample_2/File1_R1.bam.done: $(SAI_TMP)/fastq/Sample_2/File1_R1.sai.done $(SAI_TMP)/fastq/Sample_2/File1_R2.sai.done
	mkdir -p $(@D)
	@echo `date +'%F.%H:%M:%S'`" ($(BWA_EXE) sampe -r "@RG	ID:RGID2	SM:SampleID2	LB:Lib2	CN:UM	PL:ILLUMINA" $(REF) $(basename $^) test/align/fastq/Sample_2/File1_R1.fastq.gz test/align/fastq/Sample_2/File1_R2.fastq.gz | $(SAMTOOLS_EXE) view -uhS - | $(SAMTOOLS_EXE) sort -m $(BWA_MAX_MEM) - $(basename $(basename $@))) 2> $(basename $(basename $@)).sampe.log"
...
$(SAI_TMP)/fastq/Sample_2/File1_R1.sai.done:
	mkdir -p $(@D)
	@echo `date +'%F.%H:%M:%S'`" $(BWA_EXE) aln $(BWA_QUAL) $(BWA_THREADS) $(REF) test/align/fastq/Sample_2/File1_R1.fastq.gz -f $(basename $@) 2> $(basename $@).log"
...
$(SAI_TMP)/fastq/Sample_2/File1_R2.sai.done:
	mkdir -p $(@D)
	@echo `date +'%F.%H:%M:%S'`" $(BWA_EXE) aln $(BWA_QUAL) $(BWA_THREADS) $(REF) test/align/fastq/Sample_2/File1_R2.fastq.gz -f $(basename $@) 2> $(basename $@).log"
...
$(ALN_TMP)/fastq/Sample_2/File2_R1.bam.done: $(SAI_TMP)/fastq/Sample_2/File2_R1.sai.done $(SAI_TMP)/fastq/Sample_2/File2_R2.sai.done
	mkdir -p $(@D)
	@echo `date +'%F.%H:%M:%S'`" ($(BWA_EXE) sampe -r "@RG	ID:RGID2	SM:SampleID2	LB:Lib2	CN:UM	PL:ILLUMINA" $(REF) $(basename $^) test/align/fastq/Sample_2/File2_R1.fastq.gz test/align/fastq/Sample_2/File2_R2.fastq.gz | $(SAMTOOLS_EXE) view -uhS - | $(SAMTOOLS_EXE) sort -m $(BWA_MAX_MEM) - $(basename $(basename $@))) 2> $(basename $(basename $@)).sampe.log"
...
$(SAI_TMP)/fastq/Sample_2/File2_R1.sai.done:
	mkdir -p $(@D)
	@echo `date +'%F.%H:%M:%S'`" $(BWA_EXE) aln $(BWA_QUAL) $(BWA_THREADS) $(REF) test/align/fastq/Sample_2/File2_R1.fastq.gz -f $(basename $@) 2> $(basename $@).log"
	@$(BWA_EXE) aln $(BWA_QUAL) $(BWA_THREADS) $(REF) test/align/fastq/Sample_2/File2_R1.fastq.gz -f $(basename $@) 2> $(basename $@).log || (echo "`grep -i -e abort -e error -e failed $(basename $@).log`" >&2; echo "Failed aln step" >&2; mkdir -p $(OUT_DIR)/failLogs; cp $(basename $@).log $(OUT_DIR)/failLogs/$(notdir $(basename $@).log); echo "See $(OUT_DIR)/failLogs/$(notdir $(basename $@).log) for more details" >&2; exit 1;)
	rm -f $(basename $@).log
	@echo `date +'%F.%H:%M:%S'` touch $@; touch $@
...
$(SAI_TMP)/fastq/Sample_2/File2_R2.sai.done:
	mkdir -p $(@D)
	@echo `date +'%F.%H:%M:%S'`" $(BWA_EXE) aln $(BWA_QUAL) $(BWA_THREADS) $(REF) test/align/fastq/Sample_2/File2_R2.fastq.gz -f $(basename $@) 2> $(basename $@).log"
