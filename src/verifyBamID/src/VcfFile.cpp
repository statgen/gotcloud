...
const char* VcfMarker::getInfoValue(const char* key) {
  for(int i=0; i < asInfoKeys.Length(); ++i) {
    if ( asInfoKeys[i].Compare(key) == 0 ) {
      return asInfoValues[i].c_str();
    }
  }
  return NULL;
}

double VcfMarker::getInfoAlleleFrequency() {
  // check if AF field exist
  for(int i=0; i < asInfoKeys.Length(); ++i) {
    if ( asInfoKeys[i].Compare("AF") == 0 ) {
      return atof(asInfoValues[i].c_str());
    }
  }
  ...
  double AN = -1;
  for(int i=0; i < asInfoKeys.Length(); ++i) {
    if ( asInfoKeys[i].Compare("AN") == 0 ) {
      AN = atoi(asInfoValues[i].c_str());
    }    
    if ( asInfoKeys[i].Compare("AC") == 0 ) {
      AC = atoi(asInfoValues[i].c_str());
    }    
  }

  if ( ( AC >= 0 ) && ( AN > 0 ) ) {
    return (double)AC/(double)AN;
  }
  else {
    return -1.;
  }
...
std::pair<int,int> VcfMarker::computeAlleleCounts() {
  std::vector<int> indices;
  for(int i=0; i < (int)vnSampleGenotypes.size(); ++i) {
    indices.push_back(i);
  }
  return computeAlleleCounts(indices);
}

std::pair<int,int> VcfMarker::computeAlleleCounts(std::vector<int>& indices) {
  int AC = 0;
  int AN = 0;
  int g1, g2;
  for(int k=0; k < (int)indices.size(); ++k) {
    ...
    if ( vnSampleGenotypes[i] == 0xffff ) {
      ...
      // haploid;
      g1 = ((vnSampleGenotypes[i] & 0x7f00) >> 8);
      ++AN;
      if ( g1 > 0 )
	++AC;
    }
