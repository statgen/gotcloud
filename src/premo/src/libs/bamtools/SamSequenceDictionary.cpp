...
/*! \fn int SamSequenceDictionary::IndexOf(const std::string& sequenceName) const
    \brief Returns index of sequence name, -1 if not found

    \note A sequenceName of "*" always returns -1

    \param[in] sequenceName name of desired sequence entry
    \sa Contains()
*/
int SamSequenceDictionary::IndexOf(const std::string& sequenceName) const {
    if ( sequenceName == "*" )
        return -1;
    map<string, size_t>::const_iterator i = m_lookupData.find(sequenceName);
    if ( i == m_lookupData.end() )
        return -1;
