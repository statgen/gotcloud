...
{
    if (!mf.data) {
        mf.open(infile.c_str());
        if (!mf.data){
            fprintf(stderr, "Open GC content file %s failed!\n", infile.c_str());
            exit(1);
        }
    }
    // sanity check
    ...
    // mmap code
    uint8_t * pCount = (uint8_t*) mf.data;
    uint32_t* pContentVec = (uint32_t*) ( (char*)mf.data+ genome->sequenceLength());
    for (int i = 0; i < 10; i++){
        fprintf(stdout, "[%d] [mmap = %c] [fread = %c]\n", i, pCount[i], this->gcCount[i]);

    }
    for (unsigned int i = genome->sequenceLength() - 10; i < genome->sequenceLength(); i++){
        fprintf(stdout, "[%u] [mmap = %c] [fread = %c]\n", i, pCount[i], this->gcCount[i]);

    }
    for (int i = 0; i < 101; i++){
        fprintf(stdout, "[%d] [mmap = %c] [fread = %c]\n", i, pContentVec[i], this->gcContentVec[i]);
    }
    exit(1);
#endif
}
