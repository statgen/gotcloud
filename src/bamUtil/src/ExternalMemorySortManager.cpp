...
ExternalMemorySortManager::ExternalMemorySortManager(Bam2FastQ * tmpHost,std::string tmpBamFile,int nThread,int recordLimit, std::string tmpbedFile) {
	THREAD_NUM=nThread;
...
// bool ExternalMemorySortManager::readRecord(IFILE fin, int32_t* blockSize, char* tmpBam)
//{
//
//	 fin->ifread((char*)blockSize,sizeof(int32_t));
//	 fin->ifread(tmpBam,*blockSize);
//	return fin->ifeof();
//}
static int parseRegion(std::string& line,std::string &chr,long &begin,long &end)
{
	//chr2:100-200
	//chr2\t100\t200
	uint pos(0),pos2(0);
	if((pos=line.find(":",0))!=line.npos&&(pos2=line.find("-",0))!=line.npos)//find ':'
	{
	...
	{
		std::cerr<<"Unrecognized region format! Please use format:\"chr2:100-200\" or \"chr2\t100\t200\""<<std::endl;
		exit(EXIT_FAILURE);
	}
	return 0;
}
int ExternalMemorySortManager::FindAllBeginPointer() {
	...
	if (bedFile == "") {//create segments on the fly
		...
		for(int i=0;i!=nSQ;++i)
		{
			...
			{
				MateVectorList.push_back(new MateVectorByRN(vectorIndex,host, chrName,begin,begin+Section_Skip));
			...
			MateVectorList.push_back(new MateVectorByRN(vectorIndex,host, chrName,begin,seqLength));
			masterTmpFileList.push_back(std::string(""));
			sortHeap.push_back(tmpHeap);
			vectorIndex++;
		}
		MateVectorList.push_back(new MateVectorByRN(vectorIndex,host, std::string("*"),begin,begin+1));
		masterTmpFileList.push_back(std::string(""));
		sortHeap.push_back(tmpHeap);
		vectorIndex++;
		...
		long begin(0),end(0);
			while (getline(fin,line)) {
				parseRegion(line,chr,begin,end);
					MateVectorList.push_back(new MateVectorByRN(vectorIndex,host, chr,begin,end));
					masterTmpFileList.push_back(std::string(""));
					sortHeap.push_back(tmpHeap);
					vectorIndex++;
			}
...
int ExternalMemorySortManager::Dispatch() {
	...
	for(int i=0;i!=THREAD_NUM;++i)
		t[i]=std::thread(&ExternalMemorySortManager::ProcessVector,this,workDone);

	//std::cerr<<"Waiting for the process..."<<std::endl;
	...
	while (1) {
		...
		for (uint i = 0; i != MateVectorList.size(); ++i) {
			...
			if (workDone[i] == 0) {
				vectorIndex = i;
				workDone[i] = 1;
//				std::cerr<<"WorkDone array:";
//				for (uint j = 0; j != MateVectorList.size(); ++j) {
//					std::cerr<<workDone[j];
//				}
//				std::cerr<<std::endl;
				//mutex
...
int ExternalMemorySortManager::MergeSort(/*std::vector<std::string> tmpFileList*/) {//reading from signle tmp file(TODO:or heap), output to fianl file
	...
	for (uint i = 0; i < tmpFileList.size(); ++i) {
		SamFile *fin=new SamFile(tmpFileList[i].c_str(),SamFile::READ,&tmpHeader);
		if (!fin->IsOpen()) {
	...
	while (closedFile < nFile) {
		...
		if (tmpFin->IsEOF()) {
			...
			tmpRecord.first=tmpRecord.second->getReadName();
			masterHeap.push(
					std::make_pair(tmpRecord, tmpFin));
		}
...
int ExternalMemorySortManager::miniMergeSort(MateVectorByRN* tmpVector) {//reading from tmp file, ouput single to single tmp file (TODO:or heap)
	...
	std::cerr<<"Enter "<<vectorIndex<<"th vector's miniMergeSort stage..."<<std::endl;
	masterTmpFileList[vectorIndex]=tmpVector->tmpFileNameBase+std::to_string(vectorIndex)+"th.vector.all.tmp";
	tmpVector->tmpFile.OpenForWrite(masterTmpFileList[vectorIndex].c_str(),&tmpVector->tmpHeader);
	if(!tmpVector->tmpFile.IsOpen())
	{
		std::cerr<<"open "<<masterTmpFileList[vectorIndex]<<" failed, check if there is enough space to create file!"<<std::endl;
		exit(EXIT_FAILURE);
	}
	...
	SamFile* tmpFin = nullptr;
//initialize
	//std::cerr<<"vectorIndex:"<<vectorIndex<<"\t"<<sortHeap[vectorIndex].size()<<"\t"<<nFile<<std::endl;
	//std::cerr<<"top:"<<sortHeap[vectorIndex].top().first.first<<"\tpointer:"<<sortHeap[vectorIndex].top().first.second<<std::endl;
	//int round=0;
	while (closedFile < nFile) {
		//round++;
		//std::cerr<<"This is round:"<<round<<std::endl;
		tmpPop = sortHeap[vectorIndex].top();
		sortHeap[vectorIndex].pop();
		tmpRecord = tmpPop.first;
		if (tmpVector->prevRec == NULL) {
			tmpVector->prevRec = tmpRecord.second;
			//std::cerr<<"no previous:"<<tmpVector.prevRec<<std::endl;
		} else {
...
//					}//else
						//std::cerr << "Successfully printed " << samRec->getReadName()<<"\tand\t"<<tmpVector.prevRec->getReadName()<<"\t"<<samRec<<"\t"<<tmpVector.prevRec<<std::endl;
					...
					host->writeFastQ(
							*tmpVector->prevRec,
							host->mySecondFile, host->mySecondFileNameExt,tmpVector->myPool,false,
...
//					}
					...
					host->writeFastQ(
							*tmpVector->prevRec,
							host->myFirstFile, host->myFirstFileNameExt,tmpVector->myPool,false,
							host->myFirstRNExt.c_str());
		...
		if (tmpFin->IsEOF()) {
			...
			tmpFin->ReadRecord(tmpHeader,*tmpRecord.second);
			tmpRecord.first=tmpRecord.second->getReadName();
			sortHeap[vectorIndex].push(
					POPCORN(tmpRecord, tmpFin));
		}
