...
MateVectorByRN::MateVectorByRN(int tmpVectorIndex, Bam2FastQ* conversionHost, std::string chromosome, long CoordinateStart, long CoordinateEnd):bufferInUse(0),myNumPairs(0),count(0), SHRINK_LIMIT(SHRINK_LIMIT_PARAMETER), MAX_LIMIT(MAX_LIMIT_PARAMETER)/*,myPool(RECORD_POOL_PARAMETER)*/ {
...
//	            	host->writeFastQ(samRec, tmpFile,  UnpairedFileNameExt, myPool, true, host->mySecondRNExt.c_str());
			...
			myMateBuffer[1 - bufferInUse].push_back(
					std::make_pair(std::string(prevRec->getReadName()), prevRec));
...
//				}
				...
				host->writeFastQ(samRec, host->myFirstFile, FirstFileNameExt, myPool, false,
						host->myFirstRNExt.c_str());
				host->writeFastQ(*prevRec, host->mySecondFile, SecondFileNameExt, myPool, false,
...
//				}
				...
				host->writeFastQ(*prevRec, host->myFirstFile, FirstFileNameExt, myPool, false,
						host->myFirstRNExt.c_str());
				host->writeFastQ(samRec, host->mySecondFile, SecondFileNameExt, myPool, false,
						host->mySecondRNExt.c_str());
				++host->myNumPairs;
				myLock2.unlock();
...
int MateVectorByRN::Read(const std::string& bamFile) {//end not included
	...
	if(chr=="*")
	{
		tmpFileNameBase=std::string(host->myTmpOutBase.c_str())+std::string("_")+"unmap"+":"+std::to_string(begin)+"_"+std::to_string(end)+"_";
	}
	else
		tmpFileNameBase=std::string(host->myTmpOutBase.c_str())+std::string("_")+chr+":"+std::to_string(begin)+"_"+std::to_string(end)+"_";
	FIN.ReadBamIndex();
	//std::cerr<<"Region:"<<chr<<":"<<begin<<"-"<<end<<"\t in process"<<std::endl;
	if(!FIN.SetReadSection(chr.c_str(),begin,end))
	{
		std::cerr<<"Region:"<<chr<<":"<<begin<<"-"<<end<<"\tdoes not exist"<<std::endl;
		return 1;
	}
	SamRecord* recordPtr = myPool->getRecord();
	if(recordPtr==NULL) {std::cerr<<"Get record from myPool failed!"<<std::endl;exit(EXIT_FAILURE);}
	int alreadyRead=0;
...
int MateVectorByRN::Add(SamRecord* record) {
	if(count >= MAX_LIMIT)//ready to dump tmp files
	{
		...
		count=0;
		//restart
		myMateBuffer[bufferInUse].push_back(std::make_pair(std::string(record->getReadName()),record));
		count++;
		return 2;//finish job
	}
	...
	{
		if(prevRec!=NULL) {
			myMateBuffer[bufferInUse].push_back(std::make_pair(prevRec->getReadName(),prevRec));
			prevRec=NULL;
		}
		Sort();
...
void MateVectorByRN::Sort() {
	...
	std::sort(myMateBuffer[bufferInUse].begin(),myMateBuffer[bufferInUse].end(),RecordComparison);
}

int MateVectorByRN::DumpMateVector() {

	tmpFileName = tmpFileNameBase+std::to_string(vectorIndex)+"th.vector."+std::to_string(tmpFileIndex)+std::string("th.dump.tmp");
	tmpFileNameList.push_back(tmpFileName);

	tmpFile.OpenForWrite(tmpFileName.c_str(),&tmpHeader);
	if(!tmpFile.IsOpen())
	{
		std::cerr<<"open "<<tmpFileName<<" failed, check if there is enough space to create file!"<<std::endl;
		exit(EXIT_FAILURE);
	}
