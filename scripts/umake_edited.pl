...
foreach my $chr (@chrs) {
	...
	for(my $i=0; $i < $nsplits; ++$i) {
	    ...
	    print MAK "\t".&getMosixCmd($cmd)."\n";
	    $cmd = "/home/amkwong/umake/ext/bgzip -f $remotePrefix$beagleOut.vcf"; # "/usr/cluster/bin/bgzip -f $remotePrefix$beagleOut.vcf"; # old command changed 2012-08-14
	    print MAK "\t".&getMosixCmd($cmd)."\n";
	    $cmd = "/home/amkwong/umake/ext/tabix -f -pvcf $remotePrefix$beagleOut.vcf.gz"; # # "/usr/cluster/bin/tabix -f -pvcf $remotePrefix$beagleOut.vcf.gz"; # old command changed
	    print MAK "\t".&getMosixCmd($cmd)."\n";
	    print MAK "\n";
	}
    }
